...
def capture_response(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        with open(file_path, "w") as mock:
            json.dump(mock_data, mock, ensure_ascii=False, indent=1, sort_keys=True)
        return res
...
class MediaWikiOverloaded(MediaWiki):
    ...
    def __init__(
        ...
        """overloaded init"""
        MediaWiki.__init__(
            self, url=url, lang=lang, timeout=timeout, rate_limit=rate_limit, rate_limit_wait=rate_limit_wait
        )
...
french_site = MediaWikiOverloaded(url="https://fr.wikipedia.org/w/api.php", lang="fr")
asoiaf = MediaWikiOverloaded(url="https://awoiaf.westeros.org/api.php", lang="fr")
...
if PULL_ALL is True or PULL_GEOSEARCH is True:
    ...
    res = site.geosearch(
        title="new york city", latitude=Decimal("-9999999999.999"), longitude=Decimal("0.0"), results=22, radius=10000
    )
...
if PULL_ALL is True or PULL_PAGES is True:
    # unicode
    site.page("Jacques Léonard Muller")
